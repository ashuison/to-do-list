...
const Todoitems = ({text,id,isComplete,deleteTodo}) => {
  return (
    <div onClick={()=>{ToggleEvent(id)}} className='flex items-center my-3 gap-2'>
      <div className='flex flex-1 items-center cursor-pointer'>
        <img className='w-7' src={isComplete ? tick : not_tick}/>
        <p className={`text-slate-700 ml-4 text-[27px] decoration-slate-500 ${isComplete ? "line-through": ""}`}>{text}</p>
      </div>
